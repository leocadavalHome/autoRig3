...
class FacialRigUI(QtWidgets.QWidget):
    def __init__(self, parent=None):
        ...
        try:
            ...
            self.ui.sticky_edgeSelectPick_btn.clicked.connect(lambda par=self.ui.sticky_edgeSelect_txt:
                                                              self.setFieldFromSel(field=par, multiple=True))
            self.ui.sticky_doGuide_btn.clicked.connect(self.stickyLip_doGuide)
            self.ui.sticky_doRig_btn.clicked.connect(self.stickyLip_doRig)
            self.ui.sticky_autoSkin_btn.clicked.connect(self.stickyLip_autoSkin)
            self.ui.sticky_targetBrowse_btn.clicked.connect(lambda par=self.ui.sticky_target_txt:
                                                            self.setFieldFromSel(field=par, multiple=False))
        except:
            logger.error('ui setup failled!')
            raise

    def stickyLip_doGuide(self):
        getDictFromScene = self.ui.sticky_getGuide_chk.isChecked()

        self.stickyLip = stickyLips.StickyLips(name='stickyLips')

        pm.undoInfo(openChunk=True)
        try:
            if getDictFromScene:
                self.stickyLip.getDict()
                edges = self.getObjFromField(self.ui.sticky_edgeSelect_txt)
                self.stickyLip.doGuide(edgeLoop=edges, autoExtremes=False)
            else:
                edges = self.getObjFromField(self.ui.sticky_edgeSelect_txt)
                self.stickyLip.doGuide(edgeLoop=edges, autoExtremes=True)
        finally:
            pm.undoInfo(closeChunk=True)

    def stickyLip_doRig(self):
        semiOpenCorrect = self.ui.sticky_semiOpenCorrect_chk.isChecked()
        connectToMouthCorners = self.ui.sticky_connectToMouthCorners_chk.isChecked()

        pm.undoInfo(openChunk=True)
        try:
            self.stickyLip.doRig(iniOffset=semiOpenCorrect)

            if connectToMouthCorners:
                try:
                    if self.mouth:
                        lmouthCtrl = self.mouth.lcornerCntrl
                        rmouthCtrl = self.mouth.rcornerCntrl
                    else:
                        lmouthCtrl = pm.PyNode('L_mouthCorner_ctrl')
                        rmouthCtrl = pm.PyNode('R_mouthCorner_ctrl')
                except:
                    logging.warn('Nao conseguiu achar mouthCorner_ctrl')

                try:
                    lmouthCtrl.addAttr('sticky', at='float', k=1, dv=0, min=0)
                    rmouthCtrl.addAttr('sticky', at='float', k=1, dv=0, min=0)
                    lmouthCtrl.addAttr('offset', at='float', k=1, dv=0, min=0)
                    rmouthCtrl.addAttr('offset', at='float', k=1, dv=0, min=0)
                    lmouthCtrl.addAttr('height', at='float', k=1, dv=0, min=0, max=1)
                    rmouthCtrl.addAttr('height', at='float', k=1, dv=0, min=0, max=1)
                    lmouthCtrl.addAttr('overshoot', at='float', k=1, dv=0, min=0)

                    lmouthCtrl.sticky >> self.stickyLip.L_ctrl.sticky
                    lmouthCtrl.offset >> self.stickyLip.L_ctrl.offset
                    lmouthCtrl.height >> self.stickyLip.L_ctrl.height
                    lmouthCtrl.overshoot >> self.stickyLip.L_ctrl.overshoot
                    rmouthCtrl.sticky >> self.stickyLip.R_ctrl.sticky
                    rmouthCtrl.offset >> self.stickyLip.R_ctrl.offset
                    rmouthCtrl.height >> self.stickyLip.R_ctrl.height
                except:
                    logger.error('Nao conseguiu connectar os parametros aos controles de canto de boca!')
                    raise

        finally:
            pm.undoInfo(closeChunk=True)

    def stickyLip_autoSkin(self):
        num = self.ui.sticky_paralelEdges_spin.value()
        holdJnt = pm.PyNode(self.getObjFromField(self.ui.sticky_holdJnt_txt))
        mesh = pm.PyNode(self.getObjFromField(self.ui.sticky_target_txt))

        self.stickyLip.autoSkin(mesh=mesh, paralelLoopNum=num, holdJoint=holdJnt)
