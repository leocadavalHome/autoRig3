...
import autoRig3.tools.renamerTool as renamerTool
import autoRig3.ui.renameWidget as renameWidget
import logging
...
class DockableWidget(MayaQWidgetDockableMixin, QtWidgets.QDialog):
    def __init__(self, parent=None, coreWidgetCls=None ):
        ...
        self.setObjectName('renameWidget')
        self.setWindowTitle('REN')
        self.setLayout(self.createLayout())

    # create a widget and lay it out vertically
    def createLayout(self):
        self.main_layout = QtWidgets.QVBoxLayout()
        self.central_widget = RenamingUI(parent=self)
...
def makeCoreWidgetMain():
    ...
    deleteControl("renameWidgetWorkspaceControl")
...
class RenamingUI(QtWidgets.QWidget):
    def __init__(self, parent=None):
        super(RenamingUI, self).__init__(parent=parent)

        self.ui = renameWidget.Ui_renameDialog()
        ...
        self.ui.lineEdit.setToolTip(toolTip)
        self.ui.lineEdit.returnPressed.connect(self.runFunc)

    def runFunc(self):
        self.rnmQ = self.ui.lineEdit.text()
        pm.undoInfo (openChunk=True)
        try:
            renamerTool.oneLiner(self.rnmQ)
        finally:
