...
import autoRig3.ui.autoRig3Widget as autoRig3Widget
import autoRig3.ui.bipedUI as bipedUI
import autoRig3.ui.quadrupedUI as quadrupedUI
import autoRig3.ui.facialUI as facial
...
class DockableWidget(MayaQWidgetDockableMixin, QtWidgets.QDialog):
    def __init__(self, parent=None, coreWidgetCls=None ):
        ...
        self.setObjectName('autorigWidget')
    ...
    def createLayout(self):
        ...
        self.central_widget = autoRig3UI(parent=self)
...
def makeCoreWidgetMain():
    ...
    deleteControl("autorigWidgetWorkspaceControl")
...
class autoRig3UI(QtWidgets.QWidget):
    def __init__(self, parent=None):
        super(autoRig3UI, self).__init__(parent=parent)

        self.templateUI = None
        self.ui = autoRig3Widget.Ui_autoRig()
        ...
        self.connectTemplate()
        self.ui.template_comboBox.currentIndexChanged.connect(self.connectTemplate)
        self.createMenu()
        
        self.facialUI = facial.Facial(ui=self.ui)
        self.facialUI.uiSetup()

    def connectTemplate(self):
        templateIndex = self.ui.template_comboBox.currentIndex()

        if self.templateUI:
            pm.deleteUI(self.templateUI.objectName())

        if templateIndex == 0:  # biped
            self.templateUI = bipedUI.BipedUI(mainUI=self.ui)
        elif templateIndex == 1:  # quadruped
            self.templateUI = quadrupedUI.QuadrupedUI(mainUI=self.ui)

        self.ui.template_placeHolder.addWidget(self.templateUI)
        self.templateUI.show()

        self.ui.createGuide_btn.clicked.connect(self.templateUI.doGuideCallback)
        self.ui.createRig_btn.clicked.connect(self.templateUI.doRigCallback)
